...
From GaraRolskanet
LEFT JOIN Specialites ON upper(GaraRolskanet.type)=upper(Specialites.Libellé)
LEFT JOIN Categories ON upper(Categories.Categorie)=upper(GaraRolskanet.Categorie) and upper(Categories.Filiere)=upper(GaraRolskanet.Filiere)
LEFT JOIN Matrix ON Matrix.ID_Specialita=Specialites.ID_Specialita AND Matrix.ID_Category=Categories.ID_Category
LEFT JOIN Rolskanet ON upper(Rolskanet.Type)=upper(GaraRolskanet.Type) and upper(Rolskanet.Filière)=upper(GaraRolskanet.Filiere) AND upper(Rolskanet.[Épreuve])=upper(GaraRolskanet.Categorie)
LEFT JOIN Competitions ON upper(Competitions.[Name])=upper(Rolskanet.Manifestation )
